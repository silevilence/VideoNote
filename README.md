...
`0.1.0` 镜像存在首页空白问题：缺少 Blazor 启动脚本，`/_framework/blazor.web.js` 返回 404。该问题已在 `0.1.1` 修复，`latest` 现指向修复后的镜像；仍固定在 `0.1.0` 的部署需重新拉取 `latest`，或把 `VIDEONOTE_VERSION` 改为 `0.1.1`。原因、验证与本地重建方式见 [Docker 故障排查](docs/docker.md#首页空白与启动脚本-404)。
