...
{
    private static TaskDto Dto(AnalysisTask t) => new(t.Id, t.OriginalFileName, t.Mode, t.Status, t.StageDescription, t.CreatedAtUtc, t.ModelConfigId, t.PromptTemplateId);
    ...
    public async Task<IEnumerable<TaskDto>> List(CancellationToken ct) =>
        (await db.AnalysisTasks.AsNoTracking().OrderByDescending(t => t.CreatedAtUtc).ToListAsync(ct)).Select(Dto);

    [HttpGet("{id:guid}")]
    public async Task<ActionResult<TaskDto>> Get(Guid id, CancellationToken ct) =>
        await db.AnalysisTasks.FindAsync([id], ct) is { } t ? Ok(Dto(t)) : NotFound();
    ...
    public async Task<ActionResult<TaskDto>> Create([FromQuery] CreateTaskInput input, CancellationToken ct)
    {
        ...
        var task = new AnalysisTask { OriginalFileName = input.FileName, Mode = input.Mode, ModelConfigId = input.ModelConfigId,
            PromptTemplateId = input.PromptTemplateId, StageDescription = "已保存：功能验证任务，尚未接入分析管线。" };
        try
        {
