...
- [x] **补充 latest 镜像标签与默认部署示例**
    ...
    - 结果：2026-09-16 Actions 35068384817 成功，仅运行 latest 任务；两个标签匿名访问均返回 HTTP 200，摘要相同。Compose YAML 与 actionlint 校验通过，V0.1.0 Git tag 未改变。
...
- [x] **任务列表与详情页**（任务进展与结果展示）
    - [x] 任务列表：文件名、模式、状态、进度、耗时、创建时间，实时更新
    - [x] 任务详情：阶段进度、分步日志、最终报告渲染（Markdown），完成前实时刷新
    - [x] 失败态显示错误信息，支持删除任务
    - 验收：列表中任务状态随时间正确流转，详情页报告完成即显示，删除后列表移除

- [x] **Agent 对话**（基于理解结果的问答助手）
    - [x] 集成 Microsoft.Agents.AI：单 Assistant Agent，任务完成后于详情页启用对话区
    - [x] 上下文注入：最终报告 + 全部分段理解文本，一次注入
    - [x] 流式消息回复，对话消息持久化到数据库（刷新/重进后历史保留）
    - [x] 对话模型可在设置中指定（默认取任务分析模型）
    - 验收：对已完成任务提问可得到基于视频内容的回答（流式），历史记录刷新后保留，未完成任务对话区不可用

- [x] **配置与部署文档**（可运维性收尾）
    - [x] appsettings 配置项说明（FFmpeg 路径、工作目录、转写与模型默认值）
    - [x] 密钥管理方式说明（不硬编码）
    - [x] README：功能说明、启动步骤、样例视频使用方式
    - 验收：按 README 在干净环境（同系统）可完成部署启动，配置项有注释或文档说明

- [x] **端到端验收与打磨**（整体质量收口）
    - [x] 三种模式完整链路冒烟：上传 → 进度 → 报告 → 对话
    - [x] 错误路径处理：无字幕视频、无匹配模型、转写失败、任务取消
    - [x] 加载态/空态/异常提示完善
...
- [x] **修复 Docker 部署首页空白（代码与本机回归）**
    - 问题：已发布的 `0.1.0/latest` 镜像缺少 `blazor.web.js`；只复制 csproj 还原依赖时，SDK 未检测到 Razor 文件，没有引入 Blazor Web 静态资源包。
    - 范围：显式声明服务端需要 ASP.NET Web 静态资源，保留 Docker 分层还原；构建时检查启动脚本，新增发布首页冒烟验证并同步部署说明。
    - 验收：只复制 csproj 的干净还原包含静态资源包；发布后启动脚本 HTTP 200、首页正常渲染；原镜像提取文件可稳定复现 404。新镜像发布和 NAS 更新尚未执行，不宣称线上恢复。
    - 结果：2026-09-17 干净还原回归先失败后通过；GHCR 原始应用文件的启动冒烟返回 404，修复后按 csproj-only restore → 复制源文件 → publish --no-restore 发布，启动冒烟验证 HTTP 200、首页渲染和任务列表导航通过。本机未运行 Docker。
    - 发布结果：2026-09-17 推送 `V0.1.1`，Actions 35170132578 全部成功，`0.1.1` 与 `latest` manifest 摘要同为 `sha256:5201cd85ba7cda4afc9ff53f83b2fd32cc1211d5bdac7021bb67db8beb96adb4`；镜像应用层已含原缺失的 6 个文件。容器启动、Compose 拉取与 NAS 更新仍未实测。
